...
class Flight {
  ...
  Flight(String[] data) {
    date = data[0];                    // 例如 "1/1/2022 12:00:00 AM"
...
ArrayList<Flight> getFilteredFlights() {
  ...
  // 解析日期 (M/D/YYYY)
  int fromVal = -1;
  if (inputText3.trim().length() > 0) {
    fromVal = parseDateToInt(inputText3.trim());
  }
  int toVal = -1;
  if (inputText4.trim().length() > 0) {
    toVal = parseDateToInt(inputText4.trim());
  }

  for (Flight f : flights) {
    // 1) 机场筛选
    String originCode = f.origin.toLowerCase();
    String destCode   = f.dest.toLowerCase();
    String originCity = f.originCity.toLowerCase();
    String destCity   = f.destCity.toLowerCase();
    ...
    // 2) 日期筛选 (仅比较 "M/D/YYYY" 部分)
    String datePart = f.date.split(" ")[0]; // "1/1/2022"
    int flightVal = parseDateToInt(datePart);

    boolean matchesDate = true;
    if (fromVal != -1 && flightVal < fromVal) matchesDate = false;
    if (toVal   != -1 && flightVal > toVal)   matchesDate = false;
